...
def change_password_function(data):
    try:
        user = get_user_model().objects.filter(username = data["username"]).exists()
        if not user:
            return None, False
        user = get_user_model().objects.get(username= data["username"])
        check_password = user.check_password(data["old_password"])
        if not check_password:
            return None, False
        if not data["new_password"] == data["conf_new_password"]:
            return None, False
...
def forget_password_function(data):
    try:
        user = get_user_model().objects.filter(username = data["username"]).exists()
        if not user:
            return None, False
